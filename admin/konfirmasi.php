...
// Ambil daftar pesanan + data pengguna + metode pembayaran terbaru terkait pesanan
$queryListPesanan = mysqli_query($conn, "
    SELECT pesanan.*, pendaftar.username, 
           pm.metode, pm.nomor, pm.atas_nama
    FROM pesanan 
    JOIN pendaftar ON pesanan.id_pengguna = pendaftar.id
    LEFT JOIN pembayaran pm ON pm.user_id = pesanan.id_pengguna 
                              AND pm.created_at <= pesanan.tanggal
                            ...
                            <tbody>
                                ...
                                if (mysqli_num_rows($queryListPesanan) == 0) {
                                    ...
                                    while ($data = mysqli_fetch_assoc($queryListPesanan)) {
                                        // Tampilkan info pembayaran dengan format lengkap
                                        if (!empty($data['metode'])) {
                                            $pembayaran = $data['metode'].' - '.$data['nomor'].' - a/n '.$data['atas_nama'];
                                        } else {
                                            $pembayaran = 'COD';
                                        }
